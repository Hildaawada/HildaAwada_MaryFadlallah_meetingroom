...
describe("User Service Tests", () => {
  ...
  describe("POST /users/register", () => {
    it("should create a new user and return 201 status", async () => {
      ...
      expect(response.status).toBe(201);  // Checking for 201 status code (Created)
      expect(response.body.success).toBe(true);  // The success flag should be true
  ...
  describe("GET /users/:username/bookings", () => {
    it("should fetch user booking history", async () => {
      ...
      expect(response.body.bookings).toBeDefined();  // Ensure booking history is returned
    });
  });
});
...
afterAll(async () => {
  server.close();  // Close the server to allow Jest to exit properly
});
